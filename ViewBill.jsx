...
const ViewBill = () => {
  ...
  const handleActionClick = async (order_id, selectedAction) => {
    try {
      const response = await fetch(`http://localhost:5050/updatebill`, {
        method: 'PATCH',
        headers: {
              ...
              <td>
                {bill.bill_status === 'Unpaid' && (
                    ...
                    <Link to={`/paybill/${bill.bill_id}`}>
                      <button onClick={() => handleActionClick(bill.order_id, 'Unpaid')}>
                        Pay
                      </button>
                    </Link>
                    <button onClick={() => handleActionClick(bill.order_id, 'Rejected')}>
                      Reject
                    </button>
