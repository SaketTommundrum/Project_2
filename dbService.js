...
class DbService{
   ...
   async getBill(order_id){
      try{
         const response = await new Promise((resolve, reject) => 
            {
               const query = "SELECT * FROM users_quote,bills WHERE users_quote.order_id=?;"
               connection.query(query,[order_id], (err, results) => {
                  if(err) reject(new Error(err.message));
                  else resolve(results);
               })
 ...
 async updateBillStatus(order_id, bill_status){
   try{
      const response = await new Promise((resolve, reject) => 
         {
            const query = "UPDATE users_quote SET bill_status = ? WHERE order_id = ?"
            connection.query(query, [bill_status, order_id], (err, result) => {
               if(err) reject(new Error(err.message));
               else resolve(result.affectedRows);
            });
         });
